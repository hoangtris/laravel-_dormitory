              ...
              <div class="tab-content" id="custom-tabs-four-tabContent">
                ...
                <div class="tab-pane fade" id="room" role="tabpanel" aria-labelledby="custom-tabs-four-messages-tab">
                  <div class="row">
                    <div class="col-xl-12">
                      @if($room->status == 1)
                        Bạn vừa đặt phòng số <a href="{{ route('rooms.detail', $room->room_id) }}">#{{ $room->room_id }}</a>,
                         vui lòng chờ quản lý phòng xác nhận hoặc gọi số 03xxxxxxxx
                      @else
                        Phòng số <a href="{{ route('rooms.detail', $room->room_id) }}">#{{ $room->room_id }}</a><br>
                      @endif
                    </div>  
                  </div>
                </div>
                    ...
                    <div class="col-xl-12">
                      <table class="table header table-hover table-responsive-xl table-head-fixed text-nowrap">
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td><a href="{{ route('rooms.detail', $r->room_id) }}">#{{ $r->room_id }}</a></td>
                            <td class="text-truncate" style="max-width:250px;">{!! $r->content !!}</td>
                            <td>{{ $r->created_at }}</td>
                            <td>
                              <a onclick="return confirm('Bạn chắc chưa?')" href="{{ route('admin.review.destroy', $r->id) }}"><span class="badge badge-danger">Xóa</span></a>
                            </td>
                          </tr>
                          @endforeach
                        </tbody>
